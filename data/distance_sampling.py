...
def interval_coarse_band(low_km, high_km):
    ...
    band = int(np.searchsorted(CONDITION_DISTANCE_EDGES_KM, midpoint, side="right") - 1)
...
class ConditionBalancedSampler(Sampler):
    ...
    def __init__(
        ...
        self._eligible = np.flatnonzero(valid if self.distance_only else np.ones(len(valid), bool))
